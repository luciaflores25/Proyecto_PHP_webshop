<?php
	...
	if (($ses->checkActiveSession()) || (!empty($_POST))):
		...
		if (!$log):
			$ses->redirect("http://localhost:0090/DWES/WebShop/libs/formularioLogin.php?fracasoLogin");
		endif;
	else:
		$ses->redirect("http://localhost:0090/DWES/WebShop/nolog.php");
	endif;
	...
	if ($totalRegistros > 0) {
    	...
    	$sql = "SELECT * FROM producto WHERE tipo=\"pantalones\" ORDER BY id_producto DESC LIMIT ".$inicio."," . $regPorPag;

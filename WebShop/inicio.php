<?php
	...
	/* Comprobamos si recibimos los datos que se han introducido en el login, 
	...
	if (($ses->checkActiveSession()) || (!empty($_POST))):
		...
		// Comprobamos si se ha accedido a la cuenta
		if (!$log):
			$ses->redirect("http://landf.epizy.com/libs/formularioLogin.php?fracasoLogin");
		endif;
	else:
		// Si no se ha producido el logueo
	...
	$url = "http://landf.epizy.com/inicio.php";
	...
	if ($totalRegistros > 0) {
		...
		//examino la pagina a mostrar y el inicio del registro
    	...
    	$sql = "SELECT * FROM producto ORDER BY id_producto DESC LIMIT ".$inicio."," . $regPorPag;
